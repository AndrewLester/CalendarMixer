    ...
    def decorator(func: Callable[[Optional[int]], None]) -> Callable[[int], Any]:
        ...
        def wrapper(id: Optional[int] = None):
            ...
            if request.method == 'POST' and 'POST' in methods:
                excluded_keys = {
                    column.name for column in set(model.__table__.primary_key.columns)
                }

                # Exclude primary keys from form data because they're only placeholders
                # The real primary key is generated by SQLAlchemy
                model_instance = model(**formdata_to_dict(form_data, excluded_keys))
            ...
            if request.method == 'PUT' and 'PUT' in methods:
                model_instance = model.query.get_or_404(id)
                db.session.delete(model_instance)
                # TODO: Find a way to convert form data into a arbitrary model. Maybe
                # a from json method on the model?
                # Current issue = ** only does first level of dict into key-val
                # but subdicts (subforms) break it
                model_instance = model(**formdata_to_dict(form_data))
                db.session.add(model_instance)
                db.session.commit()
                return jsonify(), 200
